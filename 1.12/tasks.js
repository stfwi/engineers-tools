...
tasks["dist-check"] = function() {
  ...
  const version_engineerstools = fs.readfile("gradle.properties", function(line){
    if(line.trim().indexOf("version_engineerstools")!=0) return false;
  ...
  var fails = [];
  if(version_engineerstools=="") fails.push("Could not determine 'version_engineerstools' from gradle properties.");
  if(!gittags.length) fails.push("Version not tagged.");
  if(!gittags.filter(function(s){return s.indexOf(version_engineerstools.replace(/[-]/g,""))>=0}).length) fails.push("No tag version not found matching the gradle properties version.");
  if(git_remote.replace(/[\s]/g,"").indexOf("git@github.com:stfwi/engineers-tools.git(push)") < 0) fails.push("Not the reference repository.");
  if((git_branch != "develop") && (git_branch != "master")) {
    fails.push("No valid branch for dist. (branch:'"+git_branch+"')");
  } else if((git_branch == "develop") && (version_engineerstools.replace(/[^ab]/g,"")=="")) {
    fails.push("Cannot make release dist on develop branch.");
  } else if((git_branch == "master") && (version_engineerstools.replace(/[^ab]/g,"")!="")) {
...
tasks["sync-languages"] = function() {
  function load() {
    var lang_data = {};
    fs.find("./src/main/resources/assets/engineerstools/lang", '*.lang', function(f){
  ...
  for(var name in output_data) {
    fs.writefile("./src/main/resources/assets/engineerstools/lang/" + name + ".lang", output_data[name]);
  }
};
...
tasks["version-check"] = function() {
  ...
  fs.readfile("gradle.properties", function(line){
    ...
    } else if(line.search(/^[\s]*version_engineerstools[\s]*=/i) >= 0) {
      version_engineerstools = line.replace(/^[^=]+=/,"").trim();
    }
    return false;
  })
  const combined_version = version_minecraft + "-" + version_engineerstools;
  var readme_version_found = fs.readfile("readme.md", function(line){
    ...
    return m[1]==version_engineerstools;
  });
  var ok=true;
  if(!readme_version_found) {
    alert("Version 'v" + version_engineerstools + "' not found in the readme changelog.");
    ok = false;
  }
  if(!ok) {
    alert("Version data:");
